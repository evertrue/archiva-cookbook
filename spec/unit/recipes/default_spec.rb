...
describe 'archiva::default' do
  ...
  it 'links /etc/init.d/archiva to /opt/archiva/bin/archiva' do
    expect(chef_run).to create_link('/etc/init.d/archiva').with(
      to: '/opt/archiva/bin/archiva-workaround'
    )
  end
  ...
  it 'copies a workaround service file for archiva' do
    expect(chef_run).to create_cookbook_file ("/opt/archiva/bin/archiva-workaround")
  end
